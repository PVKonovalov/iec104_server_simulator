...
int integerVal(int val, int deviation) {
...
float floatVal(float val, int deviation) {
...
InformationObject make_reading(int val, int deviation, TypeID type, int ioa) {
    switch (type) {
        ...
        case M_ME_NA_1: /*  9 */ {
            return reinterpret_cast<InformationObject>(MeasuredValueNormalized_create(nullptr, ioa, floatVal(static_cast<float>(val), deviation), IEC60870_QUALITY_GOOD));
        }
        case M_ME_NB_1: /* 11 */ {
            return reinterpret_cast<InformationObject>(MeasuredValueScaled_create(nullptr, ioa, integerVal(val, deviation), IEC60870_QUALITY_GOOD));
        }
        case M_ME_NC_1: /* 13 */ {
            return reinterpret_cast<InformationObject>(MeasuredValueShort_create(nullptr, ioa, floatVal(static_cast<float>(val), deviation), IEC60870_QUALITY_GOOD));
        }
        case M_IT_NA_1: /* 15 */ {
            return reinterpret_cast<InformationObject>(IntegratedTotals_create(nullptr, ioa, it_val(val, deviation)));
        }
        case M_ME_ND_1: /* 21 */ {
            return reinterpret_cast<InformationObject>(MeasuredValueNormalizedWithoutQuality_create(nullptr, ioa, floatVal(static_cast<float>(val), deviation)));
        }
        case M_SP_TB_1: /* 30 */ {
            const long time_epoch = get_time();
            const long time_ms = time_epoch * 1000;
        ...
        case M_ME_TD_1: /* 34 */ {
            ...
            sCP56Time2a testTimestamp{};
            CP56Time2a_createFromMsTimestamp(&testTimestamp, time_ms);
            return reinterpret_cast<InformationObject>(MeasuredValueNormalizedWithCP56Time2a_create(nullptr, ioa, floatVal(static_cast<float>(val), deviation),
        ...
        case M_ME_TE_1: /* 35 */ {
            ...
            sCP56Time2a testTimestamp{};
            CP56Time2a_createFromMsTimestamp(&testTimestamp, time_ms);
            return reinterpret_cast<InformationObject>(MeasuredValueScaledWithCP56Time2a_create(nullptr, ioa, integerVal(val, deviation), IEC60870_QUALITY_GOOD, &testTimestamp));
        }
        case M_ME_TF_1: /* 36 */ {
            const long time_epoch = get_time();
            const long time_ms = time_epoch * 1000;
            sCP56Time2a testTimestamp{};
            CP56Time2a_createFromMsTimestamp(&testTimestamp, time_ms);
            return reinterpret_cast<InformationObject>(MeasuredValueShortWithCP56Time2a_create(nullptr, ioa, floatVal(static_cast<float>(val), deviation), IEC60870_QUALITY_GOOD,
                                                                                               &testTimestamp));
        }
        case M_IT_TB_1: /* 37 */ {
            const long time_epoch = get_time();
